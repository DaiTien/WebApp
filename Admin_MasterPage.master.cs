...
public partial class Admin_MasterPage : System.Web.UI.MasterPage
{
    ...
    public string adminName, count, hiddenAdmin, hiddenBooking, hiddenVetify, hiddenView, chucvu;
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["AdminLogined"] != null)
        {
            admin_User getusername = Session["AdminLogined"] as admin_User;
            if (getusername.groupuser_id == 2)
            {
                hiddenBooking = "hidden";
                hiddenVetify = "hidden";
                hiddenView = "hidden";
            }else if (getusername.groupuser_id == 3)
            {
                hiddenAdmin = "hidden";
                hiddenVetify = "hidden";
                hiddenView = "hidden";
            ...
            else if (getusername.groupuser_id == 4)
            {
                hiddenAdmin = "hidden";
                hiddenBooking = "hidden";
                hiddenView = "hidden";
            ...
            adminName = getusername.username_username;
             var checkchucvu =(from checkcuser in db.admin_Users
                     ...
                     select checkcv).SingleOrDefault();
            chucvu = (checkchucvu.groupuser_name).ToString();
            loadMenu();
        }
        else
        {
